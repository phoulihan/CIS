...
class nlp_func():
    ...
    def predict(self, the_model, vec_mod, label_dec, text_in):
        needed_text_1 = ' '.join(text_in)
        needed_text_1 = needed_text_1.split()
        needed_text_2 = [re.sub('[^A-Za-z]+', '', word).lower() for word in needed_text_1]
        need_text_done = re.sub(' +', ' ', ' '.join(needed_text_2))
        test_data = vec_mod.transform([need_text_done])
