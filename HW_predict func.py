# -*- coding: utf-8 -*-
"""
Created on Fri May 10 09:52:31 2019
...
class nlp_func():
    ...
    def predict(self, the_model, vec_mod, label_dec, path_in):
        ...
        inputted_text_tmp = open(path_in, 'r')
        inputted_text = inputted_text_tmp.readlines()
        inputted_text_tmp.close()
        text_split_pre = ' '.join(inputted_text)
        text_split_pre = text_split_pre.split()
        text_split = [re.sub('[^A-Za-z]+', '', word).lower() for word in text_split_pre]
        
        text_split_out = re.sub(' +', ' ', ' '.join(text_split))
        
        test_data = vec_mod.transform([text_split_out])
